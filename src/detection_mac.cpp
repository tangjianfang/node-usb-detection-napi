...
static std::atomic<bool> isAdded{false};
static std::atomic<bool> initialDeviceImport{true};
...
static std::condition_variable notifyDeviceHandled;
...
static void DeviceRemoved(void *refCon, io_service_t service, natural_t messageType, void *messageArgument) {
    ...
    if(messageType == kIOMessageServiceIsTerminated) {
        ...
        currentItem = item;
        isAdded = false;
        addedTsFunc.BlockingCall(item, [](Napi::Env env, Napi::Function callback, ListResultItem_t* data) {
...
static void DeviceAdded(void *refCon, io_iterator_t iterator) {
    ...
    while((usbDevice = IOIteratorNext(iterator))) {
        ...
        if(initialDeviceImport == false) {
            ...
            isAdded = true;
            addedTsFunc.BlockingCall(currentItem, [](Napi::Env env, Napi::Function callback, ListResultItem_t* data) {
                Napi::Object obj = Napi::Object::New(env);
                obj.Set("vendorId", data->vendorId);
                obj.Set("productId", data->productId);
                obj.Set("serialNumber", data->serialNumber);
...
static void WaitForDeviceHandled() {
    ...
    notifyDeviceHandled.wait(lock, []{ return deviceHandled; });
    deviceHandled = false;
}
...
static void SignalDeviceHandled() {
    std::unique_lock<std::mutex> lock(notifyMutex);
    deviceHandled = true;
    notifyDeviceHandled.notify_one();
}
